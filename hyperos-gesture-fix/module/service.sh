...
# Force gestural navigation overlays ON — run ONCE at boot, not in the loop.
# cmd overlay enable triggers a display redraw even if already enabled,
# which causes screen flicker when called repeatedly.
ensure_overlays() {
    local gestural=$(cmd overlay list 2>/dev/null | grep "com.android.internal.systemui.navbar.gestural" | grep -c "^\[x\]")
    if [ "$gestural" = "0" ]; then
        cmd overlay enable com.android.internal.systemui.navbar.gestural >/dev/null 2>&1
        log "Enabled gestural overlay"
    fi

    local gesture_line=$(cmd overlay list 2>/dev/null | grep "com.android.systemui.gesture.line.overlay" | grep -c "^\[x\]")
    if [ "$gesture_line" = "0" ]; then
        cmd overlay enable com.android.systemui.gesture.line.overlay >/dev/null 2>&1
        log "Enabled gesture line overlay"
    fi
}

# Run overlays ONCE at boot
ensure_overlays

# Initial settings check
ensure_gestures

# Monitor settings only (lightweight, no display redraw)
while true; do
    ensure_gestures
    sleep 5
done
